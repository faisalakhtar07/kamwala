          ...
          <div className="space-y-3">
            ...
            <button onClick={() => setUserType('worker')} className="w-full flex items-center gap-4 bg-white border border-cloud-200 rounded-card p-5 shadow-soft hover:border-brand-300 hover:bg-cloud-50 transition-colors text-left">
              ...
              <div>
                ...
                <p className="text-sm text-ink-500">Sign up and get your dashboard instantly — no waiting for approval.</p>
              </div>
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-cloud-50 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-sm">
        ...
        <div className="mb-6">
          <h1 className="font-display font-bold text-xl">{userType === 'worker' ? 'Become a Worker' : 'Create your account'}</h1>
          <p className="text-sm text-ink-500 mt-1">
            {userType === 'worker' ? 'Set up your service profile — your dashboard is ready the moment you sign up.' : 'Book trusted workers for any work, managed end to end.'}
          </p>
        </div>
        <form onSubmit={handleSubmit} className="bg-white border border-cloud-200 rounded-card p-6 shadow-soft space-y-4">
          ...
          <Input label="Mobile number" type="tel" inputMode="numeric" maxLength={10} value={form.mobile}
          ...
          <div className="relative">
            <Input label="Password" type={showPassword ? 'text' : 'password'} value={form.password} onChange={update('password')} />
          ...
          <Input label="Confirm password" type={showPassword ? 'text' : 'password'} value={form.confirmPassword} onChange={update('confirmPassword')} />
          ...
          {userType === 'worker' ? (
            <>
              ...
              <Input label="Skills / services (comma separated)" placeholder="Wiring, Fan installation" value={form.services} onChange={update('services')} />
              <Input label="Experience (years)" type="number" min="0" value={form.experienceYears} onChange={update('experienceYears')} />
              ...
              <Input label="Primary service pincode" inputMode="numeric" placeholder="e.g. 824101" value={form.primaryPincode} onChange={updatePincode('primaryPincode')} />
              <Input label="Additional service pincodes (comma separated, optional)" placeholder="824102, 824103" value={form.servicePincodes} onChange={update('servicePincodes')} />
            </>
          ) : (
            <Input label="Pincode (optional, helps us find workers near you)" inputMode="numeric" placeholder="e.g. 824101" value={form.pincode} onChange={updatePincode('pincode')} />
          )}

          {error && <p className="text-sm text-rose-500">{error}</p>}
          <Button type="submit" className="w-full" disabled={loading}>
            {loading ? 'Creating account…' : userType === 'worker' ? 'Create worker account' : 'Create account'}
          </Button>
